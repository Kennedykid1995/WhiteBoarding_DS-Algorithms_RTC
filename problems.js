...
class singlyLinkedList {
  ...
  insert(index, val) {
    ...
    if (index === 0) return !!this.unshift(val); //beginning //!! turns outcome into a boolean
  ...
  remove(index) {
    ...
    //set the next property on that node to be the next od the next node
  ...
  reverse() {
    ...
    //set the node variable to be the value of the next varible
    let node = this.head;
    this.head = this.tail;
    this.tail = node;
    let prev = null; //the end of our list has to be null
    let next;
  ...
  print() {
    //used for the reverse just to see the whole list reversed
    let arr = [];
    let current = this.head;
    while (current) {
...
class doublyLinkedList {
  ...
  push(val) {
    ...
    //if the head property is null set the head and the
    //tail to be the newly created node
    //if not set the next property on the tail to be the node
    //set the prev property on the newly created node
    //to be the tail
  ...
  pop() {
    //if there is no tail, head, return undefined
  ...
  shift() {
    ...
    //check if the length is 0 or no head and return undefined
    //store the current head in a var
    //if the length is one set the head and tail to null
  ...
  get(index) {
    //accessing a node in a doubly linked list by position
    //if the index is less than 0 or greater or equal to the
    //length return null
    //if the length is less than or equal to the length of the list
  ...
  set(index, val) {
    //replacing the value of a node to the doubly linked list
  ...
  insert(index, val) {
    ...
    //use get to access the index -1;
    //set the next and prev properties on the corrent nodes to link everything
    //increment the length
    //return true
    ...
    beforeNode.next = newNode; //connections
    newNode.prev = beforeNode; //5(bN)<-> 7(aN) insert 6(nN)
    newNode.next = afterNode; //5(bN) <-> 6(nN) <-> 7(aN)
